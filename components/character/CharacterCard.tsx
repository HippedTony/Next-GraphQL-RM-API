...
function CharacterCard({ character }: CharacterProps) {
  return (
    <Link href={`/character/${character.id}`}>
      <article className="group h-full shadow-2xl ring-1 ring-white/5 hover:-translate-y-1 hover:shadow-[0_30px_60px_-30px_rgba(14,165,233,0.65)]">
        <div className="overflow-hidden rounded-t-4xl bg-slate-950">
          <Image
            ...
            width={400}
            height={400}
            className="h-72 w-full group-hover:scale-105"
          />
        </div>
        ...
        <div className="flex flex-col justify-between space-y-4 p-6">
          <div>
            ...
            <p className="mt-2 text-sm tracking-[0.25em] text-slate-500 uppercase">
              {character.species}
            </p>
          </div>
